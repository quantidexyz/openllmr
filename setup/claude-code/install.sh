#!/usr/bin/env bash
# openllm-self-sha256: b3e2cd087fbfb461e8d63b79a5d10c2bb7e38ea4be18c8beb63e33c895f3759d
...
record_install_stamp() {
  ...
  tmp="$dir/.setup-claude-code.$$"
  (umask 077; printf '%s' "$OPENLLM_SELF_SHA256" > "$tmp" && mv -f "$tmp" "$dir/setup-claude-code") 2>/dev/null || { rm -f "$tmp"; true; }
  ...
  tmp="$dir/.setup-claude-code.version.$$"
  (umask 077; printf '%s' "1.6.39-registry.7" > "$tmp" && mv -f "$tmp" "$dir/setup-claude-code.version") 2>/dev/null || { rm -f "$tmp"; true; }
}
clear_install_stamp() {
...
read_install_version() {
  local value
  value="$(cat "$HOME/.openllm/installed/setup-claude-code.version" 2>/dev/null || true)"

...
icon: cpu
requires_key: true
features: ["Downloads + verifies the openllmd binary for your OS/arch against its published SHA-256 sidecar (a checksum, not a detached signature)", "Puts `openllmd` on your PATH so you can `openllmd start | stop | status` (with shell completion)", "Runs subscription inference locally via the official vendor CLIs' own credentials", "Installs a launch agent (macOS) / systemd user unit (Linux) so it runs headless — auto-restarts on crash and survives logout + reboot (enables linger on Linux)", "Pairs to the selected API key (written locally, mode 0600) — re-run any time to reconfigure with a different key"]
requirements: ["macOS or Linux (no Windows)", "The official vendor CLI for each subscription you use (Claude Code / Codex / Kimi)", "curl + shasum/sha256sum for checksum verification"]
---

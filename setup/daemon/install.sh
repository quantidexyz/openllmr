#!/usr/bin/env bash
# openllm-self-sha256: 2bd72ae9394fce26d29175355afe5d8f20b7dd93c29e6eb539c72fc8682278ad
...
record_install_stamp() {
  ...
  tmp="$dir/.setup-daemon.version.$$"
  (umask 077; printf '%s' "1.6.39-registry.7" > "$tmp" && mv -f "$tmp" "$dir/setup-daemon.version") 2>/dev/null || { rm -f "$tmp"; true; }
}
clear_install_stamp() {
...
read_install_version() {
  local value
  value="$(cat "$HOME/.openllm/installed/setup-daemon.version" 2>/dev/null || true)"
